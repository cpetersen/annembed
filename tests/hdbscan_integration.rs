...
fn build_test_hnsw(data: &[Vec<f32>]) -> Hnsw<f32, DistL2> {
    ...
    let max_nb_conn = 16.min(nb_data / 4).max(2);
    let ef_c = 200.min(nb_data * 2).max(10);
